...
class EN1(torch.nn.Module):
    def __init__(self, vector_size):
        ...
        self.l4 = torch.nn.Linear(len_after_AE, len_after_AE * 3)
        self.bn4 = torch.nn.BatchNorm1d(len_after_AE * 3)

        self.l5 = torch.nn.Linear(len_after_AE * 3, self.vector_size)
    ...
    def forward(self, X):
        ...
        X_AE = self.dr(self.bn4(self.ac(self.l4(X))))
        X_AE = self.l5(X_AE)
...
class EN2(torch.nn.Module):
    def __init__(self, vector_size):
        ...
        self.l6 = torch.nn.Linear(len_after_AE, len_after_AE*3)
        self.bn6 = torch.nn.BatchNorm1d(len_after_AE*3)

        self.l7 = torch.nn.Linear(len_after_AE*3, self.vector_size*2)
    ...
    def forward(self, X):
        ...
        X_AE = self.dr(self.bn6(self.ac(self.l6(X))))
        X_AE = self.l7(X_AE)
...
class FraEN(torch.nn.Module):
    def __init__(self, input_dim):
        ...
        self.l4 = torch.nn.Linear(len_after_AE, len_after_AE*2)
        self.bn4 = torch.nn.BatchNorm1d(len_after_AE*2)

        self.l5 = torch.nn.Linear(len_after_AE*2, input_dim)
...
class My_Model(nn.Module):
    def __init__(self, input_dim, n_heads, n_layers, event_num,in_features, hidd_dim, heads_out_feat_params, blocks_params, fra_dim):
        ...
        self.layers = torch.nn.ModuleList([EncoderLayer(len_after_AE * 4, n_heads) for _ in range(n_layers)])
        self.AN = torch.nn.LayerNorm(len_after_AE * 4)

        self.l1 = torch.nn.Linear(len_after_AE * 4, len_after_AE)
        self.bn1 = torch.nn.BatchNorm1d(len_after_AE)

        self.l2 = torch.nn.Linear(len_after_AE, len_after_AE//2)
        self.bn2 = torch.nn.BatchNorm1d(len_after_AE//2)

        self.l3 = torch.nn.Linear(len_after_AE//2, 65)
        ...
        self.l7 = torch.nn.Linear(len_after_AE, len_after_AE*3)
        self.bn7 = torch.nn.BatchNorm1d(len_after_AE*3)

        self.l8 = torch.nn.Linear(len_after_AE*3, input_dim)
    ...
    def forward(self, X, triples, X_fra):
        ...
        X4_AE = self.dr(self.bn7(self.ac(self.l7(X4))))
        X4_AE = self.l8(X4_AE)
        ...
        return X,X1_AE,X2_AE,X3_AE,X4_AE
